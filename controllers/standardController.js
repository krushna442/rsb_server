...
export const listStandards = async (req, res) => {
  try {
    ...
    let versionMap = {};
    if (isAdmin) {
      const allRows = await query(`SELECT id, standard_no, version, rev_number, rev_date, created_at, remarks FROM standards ORDER BY standard_no ASC, version ASC`);
      allRows.forEach(r => {
        if (!versionMap[r.standard_no]) versionMap[r.standard_no] = [];
        versionMap[r.standard_no].push({ ...r, remarks: parseRemarks(r.remarks) });
      });
    }
...
export const addStandard = async (req, res) => {
  try {
    ...
    const result = await execute(
      ...
      [standard_no, description || null, rev_number || null, rev_date || null, comment || null, file_path, cat, newVersion, serializeRemarks(remarks), createdBy]
    );
...
export const addStandardVersion = async (req, res) => {
  try {
    ...
    const result = await execute(
      ...
      [existing.standard_no, existing.description, rev_number || existing.rev_number, rev_date || new Date().toISOString().slice(0, 10), comment || existing.comment, file_path || existing.file_path, existing.category, newVersion, existing.id, serializeRemarks(remarks), createdBy]
    );
    const newRow = await queryOne('SELECT * FROM standards WHERE id = ?', [result.insertId]);
    newRow.remarks = parseRemarks(newRow.remarks);
    res.status(201).json({ success: true, data: newRow });
